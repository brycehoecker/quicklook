...
savedir = "/data/analysis_output/camera_images/quicklook_run{}/".format(runID)
try:
    os.mkdir(savedir)
except:
    pass
...
#eventList = [choose_event+k for k in range(1)]
...
for choose_event in range(nEvents):
    print("Now plotting event: ", choose_event)
    for modInd in range(len(modList)):
        for asic in range(nasic):
            for ch in range(nch):
                ievt = choose_event
                rawdata = reader.GetEventPacket(ievt, int((((nasic*modInd+asic)*nch)+ch)/chPerPacket))
                packet = target_driver.DataPacket()
                packet.Assign(rawdata, reader.GetPacketSize())
    ...
    # Perform INFN / 2 to account for different gain levels
    ...
    # set up plotting
    counter = 0
    if np.amax(physHeatArr[:,:,:]) > 400:
        counter += sum(sum(sum(physHeatArr > 400))) #This and the preceeding line implement a very basic cut on baseline subtracted pSCT data: 1) Is there a pixel with magnitude greater than 400? and 2) If so, how many?
        #If there are more than 3 pixels with a magnitude over 400, go ahead and plot the image.
        if counter >= 3:
