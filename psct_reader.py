...
class WaveformArrayReader:
    ...
    def __init__(self, file_, tracking_file=None, observing_mode="ON/OFF", source=None):
        ...
        try:
            ...
            self.__run = int(self.file_.split("/")[-1].split(".")[0].lstrip("runcaled").rstrip("_pre"))
    ...
    def get_event(self, ev, interpolate=True):
        ...
        :param interpolate: bool = True, whether or not to interpolate for
                            removed spikes that have been tagged
        """
        ...
        if self.__data_format == "r1":
            ...
            raise RawDataException                                  #This is where it breaks when trying to open fits file locally (exception on LINE 37) - Bryce Hoecker
        self.__reader.GetTimeStamp(ev)
        self.__cpu_s = self.__reader.fCPU_s
        self.__cpu_ns = self.__reader.fCPU_ns
        self.__tack = self.__reader.fTACK_time
    ...
    def get_flasher_events(self,
                           rate: float,
                           timestamps: List = []) -> List:
        """
        Process the entire data file and return a list of flasher candidates
        at the specified rate. Can optionally return subrun divisions.

        :param rate: float, specify the rate of flasher events in a data run
        :param timestamps: List, list of all TACK times of a run
        ...
        if timestamps == []:
            ...
            for ev in range(self.n_events):
                self.get_event(ev)
                tacks.append(self.tack)
            timestamps = np.asarray(tacks)
            timestamps -= timestamps[0]
            timestamps = [time / 1_000_000_000 for time in timestamps]
        opt_pers = []
        subruns = utils.list_subruns(timestamps)
        for subrun in subruns:
            opt_pers.append(utils.scan_rates(rate, subrun))
        flasher_events = utils.get_flasher_events(subruns, opt_pers)
        return flasher_events

    def get_first_good_subrun(self,
                              timestamps: List = []) -> Tuple:
        """
        Either constructs list of timestamps to feed to subrun identifier
        or takes one as optional input. Finds the first good subrun (more than
        5 events) and returns a tuple containing the first and last (inclusive)
        events in the subrun.
        ...
        for subrun in subruns:
            if len(subrun) > 5:
                return (first, first + len(subrun) - 1)
            else:
                first += len(subrun)
